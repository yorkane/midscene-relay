/**
 * Relay Server - runs on machine A alongside Chrome.
 *
 * 1. Connects to local Chrome via CDP (localhost:9222)
 * 2. Starts a Socket.IO server on 0.0.0.0:3766 (for Midscene SDK)
...
interface RelayConfig {
  ...
  port?: number;
  /** CDP reverse proxy port for Playwright/Puppeteer, default 9223 */
  cdpProxyPort?: number;
}
...
export class RelayServer {
  ...
  async start(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      httpServer.once('listening', () => {
        ...
        console.log(`[Relay] Remote SDK can connect to ws://<this-ip>:${port}`);
        resolve();
      });
      httpServer.once('error', (err) =>
        reject(new Error(`Listen error: ${err.message}`)),
      );
  ...
  private handleConnection(socket: Socket) {
    if (this.commanderSocket?.connected) {
      console.log('[Relay] Refusing new connection: already have a commander');
      socket.emit(BridgeEvent.Refused, 'Another client is already connected');
      socket.disconnect();
      return;
    }

    console.log('[Relay] Commander connected');
    this.commanderSocket = socket;

    socket.emit(BridgeEvent.Connected, { version: '1.0.0-relay' });
    ...
    socket.on('disconnect', (reason) => {
      console.log(`[Relay] Commander disconnected: ${reason}`);
      this.commanderSocket = null;
    });
  }

  /**
   * Execute a bridge call method against Chrome via puppeteer.
   */
  private async executeCall(method: string, args: any[]): Promise<any> {
    ...
    switch (method) {
      ...
      default: {
        console.warn(`[Relay] Unknown method: ${method}`);
        return undefined;
      }
    }
  }
  ...
  private async getElementsNodeTree(): Promise<any> {
    ...
    // We use the same script the Chrome extension uses
    const scriptUrl =
  ...
  /**
   * Start a CDP reverse proxy so Playwright / puppeteer can connect via
   * chromium.connectOverCDP('http://A_IP:<cdpProxyPort>').
   *
   * Proxies:
   *  - HTTP /json/* endpoints (rewrites localhost URLs to relay host)
   *  - WebSocket connections to devtools endpoints
   */
  async startCdpProxy(): Promise<void> {
    ...
    const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      // Forward /json/* HTTP requests to Chrome CDP
      ...
      try {
        ...
        // Rewrite localhost references so the remote client can connect through relay
        const localWs = `ws://${cdpOrigin.hostname}:${cdpOrigin.port}`;
        const localHttp = `http://${cdpOrigin.hostname}:${cdpOrigin.port}`;
        // We use a placeholder that the client will resolve to the relay host
        ...
        // Try to replace with actual request host if available
    ...
    // WebSocket proxy: forward devtools WS connections to Chrome
    ...
    await new Promise<void>((resolve, reject) => {
      server.once('listening', () => {
        ...
        console.log(`[CDP Proxy] Playwright: chromium.connectOverCDP('http://<this-ip>:${proxyPort}')`);
        resolve();
      });
